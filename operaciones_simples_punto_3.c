...
#include <stdlib.h>
#include <string.h>
#include "tinyexpr.h" // https://github.com/codeplea/tinyexpr
...
int main(int argc, char *argv[]) {
    char cadena[256];  // Cadena de entrada.

    // Verificamos si se pasó la cadena por línea de comando.
    if (argc < 2) {
        printf("Por favor ingrese la cadena a analizar como argumento.\n");
        return 1;
    }

    // Tomamos la cadena de la línea de comando.
    strncpy(cadena, argv[1], 255);
    cadena[255] = '\0';  // Aseguramos que la cadena esté terminada en null.

    // Verificamos si la cadena contiene caracteres inválidos.
    if (verifica(cadena)) {
        printf("La cadena contiene caracteres que no pertenecen al alfabeto permitido.\n");
        return 1;
    }

    // Contadores para cada grupo de constantes inicializados.
    int decimales = 0, octales = 0, hexadecimales = 0;

    // Verificamos si la cadena contiene números válidos según el autómata.
    if (esPalabra(cadena, &decimales, &octales, &hexadecimales)) {
        printf("Es una palabra del lenguaje.\n");
    ...
    // Realizamos la operación aritmética si solo contiene decimales.
    if (decimales > 0 && octales == 0 && hexadecimales == 0) {
        double resultado = te_interp(cadena, 0); // Función que se encarga de evaluar una cadena de números en formato string y realizar las operaciones aritméticas correspondientes.

        printf("El resultado de la operacion es: %.2f\n", resultado);
    } else {
        printf("La operacion solo puede realizarse con numeros decimales.\n");
    }
...
int verifica(char* cadena) {
    ...
    for (i = 0; cadena[i]; i++) {
        ...
        if (!(cadena[i] == '+' || cadena[i] == '-' || cadena[i] == '#' || isdigit(cadena[i]) || (cadena[i] >= 'A' && cadena[i] <= 'F') ||
              (cadena[i] >= 'a' && cadena[i] <= 'f') || cadena[i] == 'x' || cadena[i] == 'X' || cadena[i] == '*' || cadena[i] == '/')) {
